...
#' @param min_year Numeric or NULL. Minimum year to include in analysis. If NULL, no lower bound is applied.
#'   Use with max_year to filter data to specific time ranges (e.g., min_year = 1995, max_year = 2019).
#'   Default: NULL (uses all available years)
#' @param max_year Numeric or NULL. Maximum year to include in analysis. If NULL, no upper bound is applied.
#'   Use with min_year to filter data to specific time ranges (e.g., min_year = 1995, max_year = 2019).
#'   Default: NULL (uses all available years)
#' @param parallel Logical. Whether to parallelize across grid specifications. Default: FALSE (recommended).
#'   Setting to TRUE parallelizes the grid search, but requires n_specs × n_cores_per_sim total cores.
...
#'
#' This function supports two levels of parallelization, but typically only ONE should be used:
#'
#' 1. **Monte Carlo simulation level** (RECOMMENDED): Register a parallel backend with
#'    \code{doParallel} before calling this function. Each specification will run its
#'    Monte Carlo simulations in parallel. This is the most efficient approach for typical hardware.
#'
#' 2. **Grid specification level**: Set \code{parallel = TRUE} to run different specifications
#'    in parallel. This requires \code{n_specs × n_cores_per_sim} total cores and is rarely practical.
#'
#' **Recommended setup:**
#' \preformatted{
#' library(doParallel)
...
run_power_grid <- function(data_clean,
  ...
  if (length(outcome) == 0) {
    stop("outcome vector cannot be empty - must provide at least one outcome variable")
  }

  # Validate outcomes exist in data
  missing_outcomes <- setdiff(outcome, names(data_clean))
  ...
  # Loop over outcomes
  for (outcome_idx in seq_along(outcome)) {
    outcome_var <- outcome[outcome_idx]
    cat(sprintf("\n=== Processing outcome %d/%d: %s ===\n",
                outcome_idx, length(outcome), outcome_var))
    ...
    run_single_spec <- function(spec_row) {
      ...
      tryCatch({
        ...
        # IMPORTANT: Extract spec_id as a local variable for parallel safety
    ...
    # percent_effect from run_power_analysis output. We only need to add transform_outcome
    # from the grid, which is not returned by run_power_analysis.
    # Using merge by spec_id ensures correct alignment.
    results_combined <- merge(
      results_combined,
      grid[, .(spec_id, transform_outcome)],
      by = "spec_id",
      all.x = TRUE
    )
