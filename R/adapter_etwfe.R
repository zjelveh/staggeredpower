...
#' ETWFE Poisson Adapter (Convenience Wrapper)
#'
#' A convenience adapter that defaults to Poisson family.
#' Equivalent to using adapter_etwfe() with family = "poisson".
#' Note: Requires pop_var and outcome_type to be specified for proper scaling.
#'
#' @return An estimator_adapter for ETWFE Poisson estimation
#' @export
adapter_etwfe_poisson <- function() {

  base_adapter <- adapter_etwfe()

  # Wrap fit function to force family to poisson
  # Need to explicitly handle all params to ensure family is passed correctly
  fit_fn <- function(data,
                     outcome_var,
                     time_var,
                     id_var,
                     ...
                     event_study = FALSE,
                     weightsname = NULL,
                     family = NULL,  # Ignored - always poisson
                     outcome_type = NULL,
                     pop_var = NULL,
                     pretrend_test = FALSE,
                     ...) {

    # Force poisson family
    base_adapter$fit(
      data = data,
      outcome_var = outcome_var,
      time_var = time_var,
  ...
  estimator_adapter(
    name = "etwfe_poisson",
    fit_fn = fit_fn,
    extract_fn = base_adapter$extract,
    requires = "etwfe"
  )
}
