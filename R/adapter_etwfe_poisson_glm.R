...
# Each D_{g,t} = 1 for treated cohort g in post-treatment year t.
# ATT = weighted average of delta_{g,t}; SE via delta method (w'Sigma_delta w).
...
adapter_etwfe_poisson_glm <- function() {
  ...
  fit_fn <- function(data,
    ...
    # D_{g,t} = 1 for each treated cohort g in each post-treatment year t >= g
    # Never-treated and pre-treatment observations get "ref" (absorbed into FEs)
    ...
    # fixest absorbs unit + time FEs via | notation
    rhs_main <- "gt_cell"
    if (!is.null(controls)) {
    ...
    # --- Compute aggregation weights (observation-proportional) ---
    ...
    weights <- n_per_cell / sum(n_per_cell)

    # --- Aggregate: ATT = sum(w_k * delta_k) ---
    att_log <- sum(weights * delta_coefs)

    # SE via delta method: SE = sqrt(w' Sigma_delta w)
    se_log <- sqrt(as.numeric(t(weights) %*% delta_vcov %*% weights))
    ...
    list(
      ...
      event_study = NULL,  # Event study aggregation not implemented yet
      raw = fit,
      metadata = metadata
    )
  }
